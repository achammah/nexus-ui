...
2. **Styling flows through tokens only.** `src/tokens/tokens.css` (the `--nx-*` canvas) → `src/styles/shadcn.css` (bridge). An app's design lock restyles the entire kit by overriding tokens. Never hardcode colors/sizes in components.
