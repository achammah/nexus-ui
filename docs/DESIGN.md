...
**Per-APP locks live in the app** (starter `docs/DESIGN.md`, chosen from rendered direction boards at project start) and restyle this entire kit by overriding tokens — never by editing components (vendored files are overwrite-on-revendor by contract, AGENTS.md).
