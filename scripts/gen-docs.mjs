...
const OURS = [
  ...
  ["blocks/workbook/WorkbookSurface.tsx", "wrapper", "WorkbookSurface · LazyWorkbookSurface", "full spreadsheet workbook (Univer) as a standalone surface: formula bar + 400+ functions, insert/delete rows & columns, cell formatting, multiple sheets, freeze + merge; token-themed chrome, dark-synced to data-theme, free-surface (host owns the snapshot via value/onChange). The engine loads ONLY behind LazyWorkbookSurface (a ~1.6MB-gz lazy chunk), so importing the block adds nothing to the eager bundle"],
  ...
  ["blocks/workbook/workbook-theme.ts", "wrapper", "deriveWorkbookTheme · accentScale · useThemeNonce", "token → Univer theme derivation: the accent primary scale from --nx-accent, resolved to rgb for Univer's canvas color service and re-derived on a data-theme / skin flip; pairs with the --univer-* chrome overrides in workbook.css that theme Univer's own toolbar and menus"],
