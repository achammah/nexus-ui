...
- **`SuggestionPanel` renders `original`/`replacement` as TEXT** — if your strings are HTML, a card would show raw markup. Set **`previewOriginal` / `previewReplacement`** on the `Suggestion` to plain-text projections; the rail displays those and the engine still folds the real strings. They are review metadata on `Suggestion` (not `InlineChange`), so the editor's inline widgets and the record `richText` path are untouched. Omit them and the panel falls back to `original`/`replacement` exactly as before.
