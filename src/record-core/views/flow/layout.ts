...
export const forceLayout = (nodes: FlowGraphNode[], edges: FlowGraphEdge[]): Record<string, XY> => {
  ...
  // kept tight so the settled graph is compact (a small blob fits at a legible
  // zoom instead of a sprawled band that fitView shrinks to slivers)
  const k = NODE_W * 1.05;
  const iterations = Math.min(600, 320 + n * 5);
  const gravity = 0.4; // strong centre pull → a compact, readable cluster (not a sprawl)
  let temp = k * 1.6; // initial max displacement per step
  ...
  for (let it = 0; it < iterations; it++) {
    ...
    let gcx = 0;
    let gcy = 0;
    for (const id of ids) { const p = pos.get(id)!; gcx += p.x; gcy += p.y; }
    gcx /= n; gcy /= n;
    ...
    // gravity — a centre pull keeps the graph compact; the vertical pull is
    // STRONGER so the settled cluster is WIDE (matching the short, wide flow
    // canvas) and fills it at a legible zoom instead of fitting tiny by height
    for (const id of ids) {
      const p = pos.get(id)!;
      const d = disp.get(id)!;
      d.x += (gcx - p.x) * gravity;
      d.y += (gcy - p.y) * gravity * 2.4;
    }
    ...
    temp = Math.max(k * 0.03, temp - cool);
  }
